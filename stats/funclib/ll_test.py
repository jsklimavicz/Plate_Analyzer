...
if __name__ == "__main__":
	...
	conc = [2, 1, 0.5, 0.25, 0.125, 0.0625, 0.03125, 0.015625, 0, 16, 8, 4, 2, 1, 0.5, 0.25, 0.125, 0.0625, 0.03125]*3
	probs = [0, 0, 0.6875, 0.846153846153846, 1, 0.857142857142857, 0.923076923076923, 0.954545454545455, 0.875, 0, 0, 0, 0, 0, 0, 0, 0.083333333333333, 0.15, 0.916666666666667]*3
	b = [1.00,-0.991,.92]
	...
	t0 = time.time()
	for i in range(n):
		ll3c(c_int(len(probs)), b_type(*b), data_type(*probs), data_type(*conc))
	t1 = time.time()
	print((t1-t0)/n*1e6)
	...
	t0 = time.time()
	for i in range(n):
		ll2c(c_int(len(probs)), b_type(*b), data_type(*probs), data_type(*conc))
	t1 = time.time()
	print((t1-t0)/n*1e6)
