...
class MerlinGrapher:
	...
	def plot_data(self):
		...
		if "x" in self.data and "lb" in self.data and "ub" in self.data and \
											self.options["PLOT_CURVE_ERROR"]:
		...
		#Data points.
		if "conc" in self.data and "probs" in self.data and \
									self.options["PLOT_DATA_POINTS"]:
			conc = self.data["conc"].copy()
			if self.options["JITTER"]: 
				conc += np.random.uniform(-self.options["JITTER_FACTOR"], 
											self.options["JITTER_FACTOR"], 
											len(self.data["conc"]))
			self.ax.plot(conc, 
				self.data["probs"], 
				...
				ls = 'None')
			#Error bars. 
			if "error_bars" in self.data and self.options["PLOT_ERROR_BARS"] and \
						self.options["ERROR_BAR_CUTOFF"] > self.data["n_trials"]:
	...
	def set_labels(self, title):
		...
		plt.yticks(ticks=np.array(range(0, 5, 1))/4., 
					labels=np.array(range(0, 101, 25)) )
	...
	def save_plot(self, name, image_dir, *args, pdf = True, 
				pdf_dir = None, close_after = True, **kwargs):
		filename = name + ".png"
