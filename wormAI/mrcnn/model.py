...
from wormAI.mrcnn import utils
...
def refine_detections_graph(rois, probs, deltas, window, config):
    ...
    def nms_keep_map(class_id):
        ...
        ixs = tf.compat.v1.where(tf.equal(pre_nms_class_ids, pre_nms_class_ids))[:, 0]
        # Apply NMS within classes
    ...
    nms_keep = tf.map_fn(nms_keep_map, unique_pre_nms_class_ids,
                         dtype=tf.int64)
    # nms_keep = tf.map_fn(nms_keep_map_all, nms_keep,
    #                      dtype=tf.int64)
...
def mrcnn_class_loss_graph(target_class_ids, pred_class_logits,
    ...
    # #TESTING
    # target_class_ids = tf.constant([[1,1,4,2,3,2,1,1,3,2,3]])
    # pred_class_logits = tf.random.uniform(shape = [1, 11, 8])
    # active_class_ids = tf.constant([[1,1,1,1,1,1,1,1]])
    ...
    #name_dict = {"background": 0, "alive": 1,"dead": 2,"moribund": 3,"egg": 4, "L2": 5, "aged_dead": 6, "artifact": 7}

    #each row i and column j is the penalty of identifying class i as class j.
    penalty_matrix = tf.constant([[
            #Bkg  Alv  Ded  Mor  Egg  L2   AgD  Art
            [0.5, 3.0, 3.0, 3.0, 3.0, 3.0, 3.0, 0.5], #background
            [3.0, 1.0, 2.5, 1.5, 2.5, 1.5, 3.0, 2.5], #alive
            [3.0, 2.2, 1.0, 1.1, 2.0, 1.2, 1.5, 2.0], #dead
            [3.0, 1.4, 1.1, 0.8, 2.0, 1.2, 2.0, 2.0], #moribund
            [3.0, 2.5, 2.2, 2.2, 1.0, 2.0, 1.2, 1.2], #egg
            [3.0, 1.5, 1.5, 1.5, 2.0, 1.2, 1.2, 1.2], #L2
            [3.0, 3.0, 1.5, 2.0, 1.2, 1.2, 1.2, 1.2], #aged_dead
            [0.5, 2.5, 2.0, 2.0, 1.2, 1.2, 1.2, 0.5]  #artifact
        ]])

    one_hot_target = tf.one_hot(target_class_ids, depth = penalty_matrix.shape[1], on_value=1.0, off_value=0.0)
    one_hot_pred = tf.one_hot(pred_class_ids, depth = penalty_matrix.shape[1], on_value=1.0, off_value=0.0)


    target_weights = tf.linalg.matmul(one_hot_target, penalty_matrix)
    weights = tf.reduce_sum(target_weights * one_hot_pred, axis=2)

    loss = tf.nn.softmax_cross_entropy_with_logits(labels=one_hot_target, logits=pred_class_logits)
